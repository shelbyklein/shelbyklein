...
import { articleRevisions } from '@/content/article-revisions';
...
export const articles = articleData.map(article => {
  const revision = articleRevisions[article.slug];
  return {
  ...article,
  ...revision,
  html: (revision?.html ?? article.html).replace(/((?:href|src)=["'])(\/(?!\/)[^"']*)(["'])/g,
    (_, before, path, after) => before + sitePath(path) + after),
};
});
export const featuredIds = ['newton','current','playcase','olympic-jerseys','steam-deck-hq','usa-archery-broadcast','tracker-trapper','vispix'];
