...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){
 ...
 <div className="case-gallery">{project.images.filter(img=>img.src!==project.cover).map((img,i)=><figure key={img.src}><a href={img.src} target="_blank" rel="noreferrer" aria-label={`Open full image: ${img.alt}`}><img src={img.src} alt={img.alt} loading="lazy"/></a><figcaption>{project.originalTitle} <span>{String(i+1).padStart(2,'0')}</span></figcaption></figure>)}</div>
